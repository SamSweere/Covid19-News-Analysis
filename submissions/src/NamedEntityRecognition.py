...
class NamedEntityRecognizer:
    ...
    def spacy_preprocessing(self, df, nlp):
        ...
        # df["nlp"] = df["body"].apply(nlp.pipe)

        # def apply_pipe(x):
        #     for doc in nlp.pipe(x, disable=["tagger"]):
        #         print(doc)
        #     # print(list(nlp.pipe(x, disable=["tagger"])))
        #     # return list(nlp.pipe(x, disable=["tagger"]))

        # df["nlp"] = df["body"].parallel_apply(apply_pipe)

        threads = mp.cpu_count()
        # for doc in nlp.pipe(df["body"], disable=["tagger"], n_threads=threads):
        #     print(doc._.has_coref)
        #     print(doc._.coref_clusters)

        # print("Threads",threads)

        # for doc in tqdm(nlp.pipe(df["body"], disable=["tagger"], n_threads=threads, batch_size=10)):
        #     # print(doc._.coref_clusters)
        #     pass

        df["nlp"] = list(nlp.pipe(df["body"], disable=["tagger"], n_threads=threads, batch_size=100))
        # df["nlp_resolved"] = df["nlp"]
        # df["nlp"] = list(tqdm(nlp.pipe(df["body"], disable=["tagger"], n_threads=threads)))

        print("Finished NLP", str(datetime.now()))
    ...
    def most_common_entities(self, df, visualize):
        ...
        """
        df_most_common = self.find_most_common_entities(df, "nlp", n_common=1, entity_type="PERSON")
